...
namespace VE3NEA.SkySSTV.Tests
{
  ...
  /// <c>SSTV_RUN_MANUAL=1</c> before <c>dotnet test</c> — no code edit needed. <paramref name="lastResult"/>
  ...
  internal sealed class ManualFactAttribute : FactAttribute
  {
    ...
    {
      if (Environment.GetEnvironmentVariable("SSTV_RUN_MANUAL") != "1") Skip = lastResult;
    }
  }
}
